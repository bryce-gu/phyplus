...
#define DEFAULT_CONN_INTV_MIN   (3)
#define DEFAULT_CONN_LATENCY    (250)
 ...
 * GLOBAL VARIABLES
 */
uint8 bt_peer_addr[B_ADDR_LEN]={0};
// uint8 bt_peer_addr0[6]={0x49,0x49,0x49,0x49,0x70,0x70};
uint8 bt_peer_addr1[B_ADDR_LEN]={0x49,0x49,0x49,0x49,0x71,0x71};
extern uint8_t bt_peer_addr0[B_ADDR_LEN];
...
uint16 SimplePrivateCentral_ProcessEvent( uint8 task_id, uint16 events )
{
    ...
    if ( events & SPC_START_DEVICE_EVT )
    {
       // LOG_ERROR("scan...\n");
        // Start the Device
        g_advRptNum = 1;
        HCI_LE_SetScanParamCmd( FALSE, 160, 160, 0, 0 );
