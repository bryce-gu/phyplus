...
int spi_transmit_s(uint8_t* tx_buf,uint8_t* rx_buf,uint8_t len)
{
    ...
    if(tx_buf) {
      ...
      {
        SPI_CHECK_TOUT(TOUT, 10, "spi_transmit_s TOUT\r\n");
    }
...
static void
spi_sleep_handler(void)
{
    spi_bus_deinit(&m_spicfg);
}
...
int
spi_bus_deinit(spilite_cfg_t* pcfg)
{
    clk_gate_disable((MODULE_e)MOD_SPI0);

    gpio_fmux_control(pcfg->sclk_pin.pin, Bit_DISABLE);
    gpio_pull_set(pcfg->sclk_pin.pin, GPIO_PULL_UP);
    gpio_dir(pcfg->sclk_pin.pin, GPIO_INPUT);

    // gpio_fmux_control(ssn_pin, Bit_DISABLE);

    gpio_fmux_control(pcfg->MOSI.pin, Bit_DISABLE);
    gpio_pull_set(pcfg->MOSI.pin, GPIO_PULL_DOWN);
    gpio_dir(pcfg->MOSI.pin, GPIO_INPUT);

    gpio_fmux_control(pcfg->MISO.pin, Bit_DISABLE);
    gpio_pull_set(pcfg->MISO.pin, GPIO_PULL_DOWN);
    gpio_dir(pcfg->MISO.pin, GPIO_INPUT);
...
int spi_bus_init(spilite_cfg_t* pcfg)
{
    ...
    gpio_pull_set(pcfg->sclk_pin.pin, GPIO_PULL_UP);
    gpio_pull_set(pcfg->MOSI.pin, GPIO_PULL_UP);
    gpio_pull_set(pcfg->MISO.pin, GPIO_PULL_UP);
...
int spi_init(void)
{
  ...
  ret = pwrmgr_register(MOD_SPI0, spi_sleep_handler, spi_wakeup_handler);
