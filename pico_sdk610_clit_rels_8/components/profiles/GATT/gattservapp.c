...
#else
// Service Changed Characteristic Properties
const uint8 serviceChangedCharProps = GATT_PROP_INDICATE;
#endif
...
#else
// GATT Attribute Table
const gattAttribute_t gattAttrTbl[] = {
	// Generic Attribute Profile
	{
		{ ATT_BT_UUID_SIZE, primaryServiceUUID }, /* type */
		GATT_PERMIT_READ,                         /* permissions */
...
bStatus_t GATTServApp_AddService( )
{
	...
	status = GATTServApp_RegisterService( (gattAttribute_t *)gattAttrTbl, GATT_NUM_ATTRS( gattAttrTbl ),
	                              		&gattServiceCBs );
	return ( status );
}
#endif
