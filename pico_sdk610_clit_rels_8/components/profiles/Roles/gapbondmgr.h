...
typedef struct
{
	uint32 gapBond_Passcode ;
	uint8 gapBond_PairingMode;
	uint8 gapBond_MITM;
	uint8 gapBond_IOCap;
	uint8 gapBond_OOBDataFlag;
	uint8 gapBond_OOBData[KEYLEN];
...
//extern void GAPBondMgr_SetParameter( void *cfg );
 ...
 */
extern bStatus_t GAPBondMgr_LinkEst( uint16 connHandle, uint8 role );
